...
typedef struct
{
	void (*ovrie)(uint8_t state);
	void (*res)(uint8_t value);
	void (*awden)(uint8_t state);
	void (*jawden)(uint8_t state);
	void (*discnum)(uint8_t value);
	void (*jdiscen)(uint8_t state);
	void (*discen)(uint8_t state);
	void (*jauto)(uint8_t state);
	void (*awdsgl)(uint8_t state);
	void (*scan)(uint8_t state);
	void (*jeocie)(uint8_t state);
	void (*awdie)(uint8_t state);
	void (*eocie)(uint8_t state);
	void (*awdch)(uint8_t value);
}STM32446ADC_CR1;
// CR2
typedef struct
{
	void (*swstart)(uint8_t state);
	uint8_t (*read_swstart)(void);
	void (*exten)(uint8_t value);
	void (*extsel)(uint8_t value);
	void (*jswstart)(uint8_t state);
	void (*jexten)(uint8_t value);
	void (*jextsel)(uint8_t value);
	void (*align)(uint8_t state);
	void (*eocs)(uint8_t state);
	void (*dds)(uint8_t state);
	void (*dma)(uint8_t state);
	void (*cont)(uint8_t state);
	void (*adon)(uint8_t state);
}STM32446ADC_CR2;
// SMPR1
typedef struct
{
...
typedef struct
{
	...
	void (*smp5)(uint8_t state);
	void (*smp5_0)(uint8_t state);
...
typedef struct
{
	void (*tsvrefe)(uint8_t state);
	void (*vbate)(uint8_t state);
