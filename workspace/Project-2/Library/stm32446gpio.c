...
/*** GPIOA ***/
void STM32446GpioAclock( uint8_t state )
{
	if(state){ RCC->AHB1ENR |= (1 << 0); } else{ RCC->AHB1ENR &= ~(1 << 0); }
}
...
void STM32446GpioAotyper( uint8_t pin, uint8_t state )
{
	state &= 1;
	if(pin < 16){
		GPIOA->OTYPER &= ~(1 << pin);
		GPIOA->OTYPER |= (state << pin);
	}
}
...
void STM32446GpioAlckr( uint8_t pin, uint8_t state )
{
	state &= 1;
	if(pin < 16){
		GPIOA->LCKR &= ~(1 << pin);
		GPIOA->LCKR |= (state << pin);
	}
}
...
/*** GPIOB ***/
void STM32446GpioBclock( uint8_t state )
{
	if(state){ RCC->AHB1ENR |= (1 << 1); } else{ RCC->AHB1ENR &= ~(1 << 1); }
}
...
void STM32446GpioBotyper( uint8_t pin, uint8_t state )
{
	state &= 1;
	if(pin < 16){
		GPIOB->OTYPER &= ~(1 << pin);
		GPIOB->OTYPER |= (state << pin);
	}
}
...
void STM32446GpioBlckr( uint8_t pin, uint8_t state )
{
	state &= 1;
	if(pin < 16){
		GPIOB->LCKR &= ~(1 << pin);
		GPIOB->LCKR |= (state << pin);
	}
}
...
/*** GPIOC ***/
void STM32446GpioCclock( uint8_t state )
{
	if(state){ RCC->AHB1ENR |= (1 << 2); } else{ RCC->AHB1ENR &= ~(1 << 2); }
}
...
void STM32446GpioCotyper( uint8_t pin, uint8_t state )
{
	state &= 1;
	if(pin < 16){
		GPIOC->OTYPER &= ~(1 << pin);
		GPIOC->OTYPER |= (state << pin);
	}
}
...
void STM32446GpioClckr( uint8_t pin, uint8_t state )
{
	state &= 1;
	if(pin < 16){
		GPIOC->LCKR &= ~(1 << pin);
		GPIOC->LCKR |= (state << pin);
	}
}
...
/*** GPIOD ***/
void STM32446GpioDclock( uint8_t state )
{
	if(state){ RCC->AHB1ENR |= (1 << 3); } else{ RCC->AHB1ENR &= ~(1 << 3); }
}
...
void STM32446GpioDotyper( uint8_t pin, uint8_t state )
{
	state &= 1;
	if(pin < 16){
		GPIOD->OTYPER &= ~(1 << pin);
		GPIOD->OTYPER |= (state << pin);
	}
}
...
void STM32446GpioDlckr( uint8_t pin, uint8_t state )
{
	state &= 1;
	if(pin < 16){
		GPIOD->LCKR &= ~(1 << pin);
		GPIOD->LCKR |= (state << pin);
	}
}
...
/*** GPIOE ***/
void STM32446GpioEclock( uint8_t state )
{
	if(state){ RCC->AHB1ENR |= (1 << 4); } else{ RCC->AHB1ENR &= ~(1 << 4); }
}
...
void STM32446GpioEotyper( uint8_t pin, uint8_t state )
{
	state &= 1;
	if(pin < 16){
		GPIOE->OTYPER &= ~(1 << pin);
		GPIOE->OTYPER |= (state << pin);
	}
}
...
void STM32446GpioElckr( uint8_t pin, uint8_t state )
{
	state &= 1;
	if(pin < 16){
		GPIOE->LCKR &= ~(1 << pin);
		GPIOE->LCKR |= (state << pin);
	}
}
...
#ifdef STM32F446xx
/*** GPIOF ***/
void STM32446GpioFclock( uint8_t state )
{
	if(state){ RCC->AHB1ENR |= (1 << 5); } else{ RCC->AHB1ENR &= ~(1 << 5); }
}
...
void STM32446GpioFotyper( uint8_t pin, uint8_t state )
{
	state &= 1;
	if(pin < 16){
		GPIOF->OTYPER &= ~(1 << pin);
		GPIOF->OTYPER |= (state << pin);
	}
}
...
void STM32446GpioFlckr( uint8_t pin, uint8_t state )
{
	state &= 1;
	if(pin < 16){
		GPIOF->LCKR &= ~(1 << pin);
		GPIOF->LCKR |= (state << pin);
	}
}
...
/*** GPIOG ***/
void STM32446GpioGclock( uint8_t state )
{
	if(state){ RCC->AHB1ENR |= (1 << 6); } else{ RCC->AHB1ENR &= ~(1 << 6); }
}
...
void STM32446GpioGotyper( uint8_t pin, uint8_t state )
{
	state &= 1;
	if(pin < 16){
		GPIOG->OTYPER &= ~(1 << pin);
		GPIOG->OTYPER |= (state << pin);
	}
}
...
void STM32446GpioGlckr( uint8_t pin, uint8_t state )
{
	state &= 1;
	if(pin < 16){
		GPIOG->LCKR &= ~(1 << pin);
		GPIOG->LCKR |= (state << pin);
	}
}
...
#endif
/*** GPIOH ***/
void STM32446GpioHclock( uint8_t state )
{
	if(state){ RCC->AHB1ENR |= (1 << 7); } else{ RCC->AHB1ENR &= ~(1 << 7); }
}
...
void STM32446GpioHotyper( uint8_t pin, uint8_t state )
{
	state &= 1;
	GPIOH->OTYPER &= ~(1 << pin);
	GPIOH->OTYPER |= (state << pin);
}
...
void STM32446GpioHlckr( uint8_t pin, uint8_t state )
{
	state &= 1;
	if(pin < 16){
		GPIOH->LCKR &= ~(1 << pin);
		GPIOH->LCKR |= (state << pin);
	}
}
