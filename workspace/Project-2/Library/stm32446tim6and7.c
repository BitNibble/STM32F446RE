...
/*** TIM6 ***/
/************/
void STM32446Tim6Clock(uint8_t state)
{
	if(state){RCC->APB1ENR |= (1 << 4);}else{RCC->APB1ENR &= ~(1 << 4);}
}
void STM32446Tim6Nvic(uint8_t state)
{ // 54
...
/************************/
// CR1
void STM32446Tim6_set_apre(uint8_t state)
{
	set_reg_block(&TIM6->CR1, 1, 7, state);
}
void STM32446Tim6_set_opm(uint8_t state)
{
	set_reg_block(&TIM6->CR1, 1, 3, state);
}
void STM32446Tim6_set_urs(uint8_t state)
{
	set_reg_block(&TIM6->CR1, 1, 2, state);
}
void STM32446Tim6_set_udis(uint8_t state)
{
	set_reg_block(&TIM6->CR1, 1, 1, state);
}
void STM32446Tim6_cen(uint8_t state)
{
	set_reg_block(&TIM6->CR1, 1, 0, state);
}
// CR2
void STM32446Tim6_mms(uint8_t value)
{
	set_reg_block(&TIM6->CR2, 3, 4, value);
}
// DIER
void STM32446Tim6_ude(uint8_t state)
{
	set_reg_block(&TIM6->DIER, 1, 8, state);
}
void STM32446Tim6_uie(uint8_t state)
{
	set_reg_block(&TIM6->DIER, 1, 0, state);
}
// SR
uint8_t STM32446Tim6_uif(void)
{
	return get_reg_block(TIM6->SR, 1, 0);
}
void STM32446Tim6_clear_uif(void)
{
	set_reg_block(&TIM6->SR, 1, 0, 0);
}
// EGR
void STM32446Tim6_ug(void)
{
	set_reg_block(&TIM6->EGR, 1, 0, 1);
}
// CNT
...
/*** TIM7 ***/
/************/
void STM32446Tim7Clock(uint8_t state)
{
	if(state){RCC->APB1ENR |= (1 << 5);}else{RCC->APB1ENR &= ~(1 << 5);}
}
void STM32446Tim7Nvic(uint8_t state)
{ // 55
...
/************************/
// CR1
void STM32446Tim7_set_apre(uint8_t state)
{
	set_reg_block(&TIM7->CR1, 1, 7, state);
}
void STM32446Tim7_set_opm(uint8_t state)
{
	set_reg_block(&TIM7->CR1, 1, 3, state);
}
void STM32446Tim7_set_urs(uint8_t state)
{
	set_reg_block(&TIM7->CR1, 1, 2, state);
}
void STM32446Tim7_set_udis(uint8_t state)
{
	set_reg_block(&TIM7->CR1, 1, 1, state);
}
void STM32446Tim7_cen(uint8_t state)
{
	set_reg_block(&TIM7->CR1, 1, 0, state);
}
// CR2
void STM32446Tim7_mms(uint8_t value)
{
	set_reg_block(&TIM7->CR2, 3, 4, value);
}
// DIER
void STM32446Tim7_ude(uint8_t state)
{
	set_reg_block(&TIM7->DIER, 1, 8, state);
}
void STM32446Tim7_uie(uint8_t state)
{
	set_reg_block(&TIM7->DIER, 1, 0, state);
}
// SR
uint8_t STM32446Tim7_uif(void)
{
	return get_reg_block(TIM7->SR, 1, 0);
}
void STM32446Tim7_clear_uif(void)
{
	set_reg_block(&TIM7->SR, 1, 0, 0);
}
// EGR
void STM32446Tim7_ug(void)
{
	set_reg_block(&TIM7->EGR, 1, 0, 1);
}
// CNT
